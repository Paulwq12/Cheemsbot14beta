...
async function ytMp4(url) {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            // Filter undefined results and check if we have a valid format
            const resultFix = result.filter(x => x.video && x.size && x.quality);
            if (resultFix.length === 0) {
                return reject(new Error("No suitable MP4 format found with both video and audio."));
            }

            const { title, description, viewCount, likes, dislikes, ownerChannelName, uploadDate, thumbnails } = getUrl.videoDetails;
            resolve({
                title,
            ...
            console.error("Error in ytMp4 function:", error);
            reject(new Error("Failed to retrieve video information. Please try a different link or check the video URL."));
        }
    });
}
